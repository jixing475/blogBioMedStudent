...
data %>% 
  ...
  plot_histogram(theme_config = theme_minimal())
```
...
data %>%
  ...
  plot_bar(theme_config = theme_light()) 
```
...
data_training <- training(init_split)
data_testing <- testing(init_split)
```
...
```{r recipe}
data_recipe <- recipe(log_price ~ ., #short-cut, . = all other vars
                       data = data) %>% 
  # Pre-processing:
...
training_data_prepared %>% 
  plot_histogram(theme_config = theme_light())
```
...
collect_metrics(data_lm_fit_cv)
...
collect_metrics(data_lm_test)

# predictions from testing data
collect_predictions(data_lm_test) 
```
...
```{r}
saveRDS(data_lm_fit, "data_lm_fit.rds")
```
...
```{r}
data_lm_read <- readRDS("data_lm_fit.rds")
```
...
```{r pred-bind-ci-new}
fake_new_data %>% 
  bind_cols(predict(data_lm_read,
                    fake_new_data)) %>% 
  bind_cols(predict(data_lm_read,
                    fake_new_data, 
                    type = "conf_int")) 
...
```{r tune_wf}
data_lasso_final_wf <- data_lasso_wf %>% 
...
```{r lasso_train}
data_lasso_final_mod <- data_lasso_final_wf %>% 
  fit(data = data_training)

data_lasso_final_mod %>% 
  pull_workflow_fit() %>% 
  tidy() 
```
...
data_lasso_test <- data_lasso_final_wf %>% 
  last_fit(init_split)
